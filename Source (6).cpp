...
public:
	...
	size_t rank() const {
		Matrix<M, N, Field> temp = *this; // Копируем матрицу
		size_t rank = 0;
		...
		for (size_t i = 0; i < N && rank < M; ++i) {
			...
			// Если максимальный элемент не равен нулю
	...
	Matrix<M, N, Field> Mreverse() const {
		...
		Field det = this->det();
		if (det == 0) {
			throw std::runtime_error("Обратной матрицы не существует (определитель равен нулю).");
		}

		// Создаем результирующую матрицу
		Matrix<M, N, Field> rez;

		// Заполняем результирующую матрицу алгебраическими дополнениями
		for (size_t i = 0; i < m; i++) {
			for (size_t j = 0; j < m; j++) {
				...
				if ((i + j) % 2 == 1) {
					minor =  minor * (-1);  // Меняем знак, если сумма индексов нечётная
				}

				// Записываем элемент обратной матрицы (сразу транспонируем)
				rez(j, i) = minor / det;
			}
		}
